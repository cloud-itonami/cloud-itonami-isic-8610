(ns hospital.operation
  "OperationActor -- one hospital operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (HospitalOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Clinical Oversight Governor
  ...
  One graph run = one hospital operation (intake -> advise -> govern
  ...
  decision to a human operator (the licensed clinician). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:treatment/administer`/`:discharge/authorize` ALWAYS reach this
  node when the governor is clean -- see `hospital.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [hospital.hospitalopsllm :as hospitalopsllm]
...
(defn build
  ...
  opts:
    :advisor      -- a `hospital.hospitalopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (hospitalopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; HospitalOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (hospitalopsllm/-advise advisor store request)]
            {:proposal p :audit [(hospitalopsllm/trace request p)]})))

      ;; Clinical Oversight Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
